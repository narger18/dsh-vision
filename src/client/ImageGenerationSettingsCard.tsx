...
import {
  ...
  draftOfImageGeneration,
  imageDraftForProvider,
  imageSettingsOps,
  isImageProviderName,
  sameImageDraft,
  validImageDraft,
...
export interface ImageGenerationSettingsCardInjected {
  readonly scope: SettingsScope<ImageGenerationSettings>
  readonly api: Pick<IApiClient, "settings" | "credentials">
}

export interface ImageGenerationSettingsCardProps
  extends Partial<ImageGenerationSettingsCardInjected> {
...
export function ImageGenerationSettingsCard(
  props: ImageGenerationSettingsCardProps
): ReactNode {
...
function Loaded({
  ...
  api,
  t,
}: ImageGenerationSettingsCardInjected & {
  readonly t: (key: VisionLocaleKey) => string
}): ReactNode {
  ...
  const [baseline, setBaseline] = useState<ImageGenerationSettings>(initial)
  const [draft, setDraft] = useState<ImageGenerationSettings>(initial)
  ...
  const maxImagesValid =
    Number.isInteger(draft.maxImages) &&
    draft.maxImages >= 1 &&
    draft.maxImages <= 8
  ...
  const discard = (): void => {
    const next = draftOfImageGeneration(
  ...
  const save = (): void => {
    ...
    void (async () => {
      ...
      if (ops.length > 0) {
        ...
        const next = draftOfImageGeneration(
          response.result.value.value as ImageGenerationSettings
        )
        setBaseline(next)
        setDraft(next)
  ...
  const spec = draft.provider !== undefined ? IMAGE_PROVIDERS[draft.provider] : undefined
  const modelOptions =
    ...
    <li className="dsh-vision-settings-card" data-open={open ? "true" : "false"}>
      ...
      >
        <span className="dsh-vision-settings-head-text">
          <span className="dsh-vision-settings-name">{t("imageGenerationTitle")}</span>
          <span className="dsh-vision-settings-description">{t("imageGenerationDescription")}</span>
        </span>
        ...
        ? (
          <div className="dsh-vision-settings-body">
            ...
            <div className="dsh-vision-settings-field">
              <label className="dsh-vision-settings-label" htmlFor="dsh-image-provider">
                {t("imageProvider")}
              </label>
              <select
                ...
                disabled={disabled}
                onChange={(event) => {
                  const provider = isImageProviderName(event.target.value)
                    ? event.target.value
                    : undefined
            ...
            {draft.provider === undefined
              ...
              : (
                <>
                  <div className="dsh-vision-settings-field">
                    <label className="dsh-vision-settings-label" htmlFor="dsh-image-model">
                      {t("imageModel")}
                    </label>
                  ...
                  <div className="dsh-vision-settings-field">
                    <label className="dsh-vision-settings-label" htmlFor="dsh-image-api-key">
                      {t("apiKey")}
                    </label>
                  ...
                  {draft.provider === "custom" && (
                    <div className="dsh-vision-settings-field">
                      <label className="dsh-vision-settings-label" htmlFor="dsh-image-credential-name">
                        {t("credentialName")}
                      </label>
                      <Input
                  ...
                  <div className="dsh-vision-settings-field">
                    <label className="dsh-vision-settings-label" htmlFor="dsh-image-base-url">
                      {t("imageBaseURL")}
                    </label>
                    <Input
                  ...
                  <div className="dsh-vision-settings-field">
                    ...
                    <p className={maxImagesValid ? "dsh-vision-settings-hint" : "dsh-vision-settings-error"}>
                      {t(maxImagesValid ? "imageMaxImagesHint" : "invalidImageMaxImages")}
                    </p>
                  </div>
                </>
              )}
            ...
            <div className="dsh-vision-toggle-row">
              <div className="dsh-vision-toggle-label">
                <span className="dsh-vision-toggle-title">{t("enableImageGeneration")}</span>
            ...
            <div className="dsh-vision-settings-footer">
              <p
                ...
                role="status"
              >
                {externalChange ? t("conflict") : failure ?? (saved ? t("imageSaved") : "")}
              </p>
              <button
                type="button"
