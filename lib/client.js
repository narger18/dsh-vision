window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function registerSettingsPluginCard(slots, component, options) {
			...
			if (target === void 0) throw new Error(`dsh-vision: unsupported settings.plugin.item slot kind "${kind ?? "missing"}"`);
...
`;
		...
		function apply(ctx) {
			...
			ctx.slots.inject("settings.plugin.item", () => registerSettingsPluginCard(ctx.slots, VisionSettingsCard, {
				namespace: VISION_SETTINGS_NAMESPACE,
				legacyId: "dsh-vision",
				legacyOrder: 30,
